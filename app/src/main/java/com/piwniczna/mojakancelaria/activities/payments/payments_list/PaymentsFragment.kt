...
class PaymentsFragment(var case: CaseEntity)  : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val caseNameTextView = view.findViewById<TextView>(R.id.payments_case_name)
    ...
    private fun openPaymentDetailsFragment(paymentPosition: Int) {
        fragmentManager?.beginTransaction()?.replace(
                R.id.fragment_container,
                PaymentDetailsFragment(case, paymentsList[paymentPosition])
        )?.commit()
    }
